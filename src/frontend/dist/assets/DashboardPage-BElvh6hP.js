import { b as commonjsGlobal, g as getDefaultExportFromCjs, r as reactExports, d as clsx, R as React, i as invariant, a as useNavigate, j as jsxRuntimeExports, S as Skeleton } from "./index-CzAnGejr.js";
import { u as useCompanyTimezone, L as Layout, f as formatDateInTz, C as ChartNoAxesColumn, S as Select, a as SelectTrigger, b as SelectValue, c as SelectContent, d as SelectItem } from "./Layout-B1HD-_-K.js";
...
function endOfWeekIso(date2, _tz) {
  const d2 = new Date(date2);
  const dow = d2.getDay();
  const daysToSunday = dow === 0 ? 0 : 7 - dow;
  d2.setDate(d2.getDate() + daysToSunday);
...
function FeriensaldoWidget() {
    ...
    queryFn: async () => {
      ...
      try {
        ...
        const vacTypeIds = new Set(
          (typesRes ?? []).filter((t2) => t2.requiresApproval).map((t2) => String(t2.id))
        );
        const filtered = (absencesRes ?? []).filter(
          (a2) => vacTypeIds.has(String(a2.absenceTypeId)) && a2.status === "approved" && String(a2.employeeId) === String(targetEmpId)
        );
  ...
  const totalGrantedDays = (vacationBalances ?? []).filter((b2) => Number(b2.kalenderjahr) === currentYear).reduce((sum, b2) => sum + Number(b2.dauer) / 100, 0);
  const usedDays = ((absenceData == null ? void 0 : absenceData.absences) ?? []).reduce((sum, a2) => {
    const yearFrom = (/* @__PURE__ */ new Date(`${a2.dateFrom}T12:00:00`)).getFullYear();
    const yearTo = (/* @__PURE__ */ new Date(`${a2.dateTo}T12:00:00`)).getFullYear();
    if (yearFrom !== currentYear && yearTo !== currentYear) return sum;
    const effectiveFrom = yearFrom < currentYear ? `${currentYear}-01-01` : a2.dateFrom;
    const effectiveTo = yearTo > currentYear ? `${currentYear}-12-31` : a2.dateTo;
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(Card, { className: "shadow-card", "data-ocid": "feriensaldo-widget", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx(CardHeader, { className: "pb-2", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-between flex-wrap gap-2", children: [
      ...
      isAdminOrManager && employees.length > 0 && /* @__PURE__ */ jsxRuntimeExports.jsxs(
        ...
        {
          value: selectedEmpId,
          onValueChange: (v2) => setSelectedEmpId(v2),
          children: [
...
function ArbeitszeitsaldoWidget() {
  ...
  const employmentStartDate = (() => {
    ...
    const earliest = validEmployments.reduce(
      (min2, emp) => emp.von < min2.von ? emp : min2
    );
    const localDate2 = nanosToLocalIsoDate(earliest.von);
    if (!localDate2) return null;
    const year = Number(localDate2.slice(0, 4));
  ...
  const { data: myEmployments = [], isLoading: employmentsLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      try {
        ...
        const r = res;
        if (r.__kind__ === "ok") {
          return (r.ok ?? []).filter((e3) => {
            if (!e3.von) return false;
            let vonBig;
            try {
              vonBig = typeof e3.von === "bigint" ? e3.von : BigInt(Math.round(Number(e3.von)));
            } catch {
              return false;
            }
